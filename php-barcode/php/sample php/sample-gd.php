<?php
  ...
  //$font     = './NOTTB___.TTF';
  $font     = '../../../_font_rotulo.ttf';
  // - -
  if($_GET['font'] ==''){ $_GET['font'] = 12; }
  if($_GET['marge'] ==''){ $_GET['marge'] = 10; }
  if($_GET['x'] ==''){ $_GET['x'] = 125; }
  if($_GET['y'] ==''){ $_GET['y'] = 125; }
  if($_GET['height'] ==''){ $_GET['height'] = 50; }
  if($_GET['width'] ==''){ $_GET['width'] = 2; }
  if($_GET['angle'] ==''){ $_GET['angle'] = 0; }
  if($_GET['code'] ==''){ $_GET['code'] = '123456789012'; }
  if($_GET['type'] ==''){ $_GET['type'] = 'code128'; }
  
  $fontSize = $_GET['font']; //10;   // GD1 in px ; GD2 in point
  $marge    = $_GET['marge']; //10;   // between barcode and hri in pixel
  $x        = $_GET['x']; //125;  // barcode center
  $y        = $_GET['y']; //125;  // barcode center
  $height   = $_GET['height']; //50;   // barcode height in 1D ; module size in 2D
  $width    = $_GET['width']; //2;    // barcode height in 1D ; not use in 2D
  $angle    = $_GET['angle']; //90;   // rotation in degrees : nb : non horizontable barcode might not be usable because of pixelisation
  
  $code     = $_GET['code']; //'123456789012'; // barcode, of course ;)
  ...
  imagefilledrectangle($im, 0, 0, 150, 75, $white);
