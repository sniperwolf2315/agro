...
$dato="";
    $fecha=date("d_m_Y");
    $miruta='informes/';
    $nombre_fichero = 'Informe_'.$tipo.'_';
             ...
             if($tipo=='Facturados'){
                ...
                //while($resultadoitems = mssql_fetch_array($resultSQLItemsm)){
                    ...
                    while($c<=$number){
                        ...
                        //count(rf.[Factura]) as FacturasDespachadasMes
                        $resultSQLItemsm2 = mssql_query("SELECT DISTINCT count(rf.[Factura]) as FacturasDespachadasMes FROM [sqlFacturas].[dbo].[facRegistroValidacion] as rv left join [sqlFacturas].[dbo].[facRegistroFactura] as rf ON rv.NumeroFactura=rf.Factura and rv.Orden=rf.Orden and (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."' AND DAY(rf.Fecha)='".$diax."') left join [sqlFacturas].[dbo].[facRegistroValidacion] as rv2 ON rf.Factura=rv2.NumeroFactura and rf.Orden=rv2.Orden and rv.IdRegistroValidacion=rv2.IdRegistroValidacion where rv.Funcionario='".$funcionario."' AND (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."' AND DAY(rf.Fecha)='".$diax."') AND rv.TipoFactura IN ('07','S2','FD','F1','D4')",$cLink);
                        if($resultadoitems2 = mssql_fetch_array($resultSQLItemsm2)){
