        ...
        //agrupamiento por funcionario
            ...
            while($resultadoitems = mssql_fetch_array($resultSQLItemsm)){
             ...
             //if($tipo=='Facturados'){
                ...
                $tam=count($Usuarios);
                $i=0;
                //$objPHPExcel->setActiveSheetIndex($m)->setCellValue("E".$f-1, "P.FACTURADOS");
                 while($i<$tam){
                //funcionarios
                //while($resultadoitems = mssql_fetch_array($resultSQLItemsm)){
                   ...
                   /* while($c<=$number){
                        ...
                        if($company=="AG"){
                            $resultSQLItemsm2 = mssql_query("SELECT DAY(rf.Fecha) as Dia, count(rf.[Factura]) as FacturasDespachadasMes FROM [sqlFacturas].[dbo].[facRegistroValidacion] as rv left join [sqlFacturas].[dbo].[facRegistroFactura] as rf ON rv.NumeroFactura=rf.Factura and rv.Orden=rf.Orden and (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') left join [sqlFacturas].[dbo].[facRegistroValidacion] as rv2 ON rf.Factura=rv2.NumeroFactura and rf.Orden=rv2.Orden and rv.IdRegistroValidacion=rv2.IdRegistroValidacion where rv.Funcionario='".$funcionario."' AND (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') AND rv.TipoFactura IN ('07','S2','FD','F1','D4') GROUP BY DAY(rf.Fecha) ORDER BY DAY(rf.Fecha) ASC",$cLink);
                        }
                        if($company=="X1"){
                            $resultSQLItemsm2 = mssql_query("SELECT DAY(rf.Fecha) as Dia, count(rf.[Factura]) as FacturasDespachadasMes FROM [sqlFacturasPestar].[dbo].[facRegistroValidacion] as rv left join [sqlFacturasPestar].[dbo].[facRegistroFactura] as rf ON rv.NumeroFactura=rf.Factura and rv.Orden=rf.Orden and (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') left join [sqlFacturasPestar].[dbo].[facRegistroValidacion] as rv2 ON rf.Factura=rv2.NumeroFactura and rf.Orden=rv2.Orden and rv.IdRegistroValidacion=rv2.IdRegistroValidacion where rv.Funcionario='".$funcionario."' AND (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') AND rv.TipoFactura IN ('01','02','04','ZB') GROUP BY DAY(rf.Fecha) ORDER BY DAY(rf.Fecha) ASC",$cLink);
                        }
                        if($company=="ZZ"){
                            $resultSQLItemsm2 = mssql_query("SELECT DAY(rf.Fecha) as Dia, count(rf.[Factura]) as FacturasDespachadasMes FROM [sqlFacturasComervet].[dbo].[facRegistroValidacion] as rv left join [sqlFacturasComervet].[dbo].[facRegistroFactura] as rf ON rv.NumeroFactura=rf.Factura and rv.Orden=rf.Orden and (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') left join [sqlFacturasComervet].[dbo].[facRegistroValidacion] as rv2 ON rf.Factura=rv2.NumeroFactura and rf.Orden=rv2.Orden and rv.IdRegistroValidacion=rv2.IdRegistroValidacion where rv.Funcionario='".$funcionario."' AND (YEAR(rf.Fecha)='".$anio."' AND MONTH(rf.Fecha)='".$mes."') AND rv.TipoFactura IN ('01','02') GROUP BY DAY(rf.Fecha) ORDER BY DAY(rf.Fecha) ASC",$cLink);
                        }
                        while($resultadoitems2 = mssql_fetch_array($resultSQLItemsm2)){
                            $d4=$resultadoitems2["FacturasDespachadasMes"];
