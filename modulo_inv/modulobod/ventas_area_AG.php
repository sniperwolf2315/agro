...
$vendext = "'VEND039', 'VEND040', 'VEND045', 'VEND078', 'VEND079', 'VEND081', 'VEND114',  'VEND165', 'VEND183', 'VEND214', 
	         'VEND252', 'VEND260', 'VEND310', 'VEND313', 'VEND314', 'VEND334', 'VEND338'";
...
while($row = $result->fetch_assoc()){
    ...
    if( $row['NOMBRE_VEND'] == '' ){
    	$vend = trim($row[CALL]);
    	$sqlN = "select SRBCTLSD.CTNAME as NOM from SRBCTLSD WHERE SRBCTLSD.CTSIGN = '$vend' ";
    	$resultN = odbc_exec($db2conp, $sqlN) ; echo odbc_errormsg();
		while($rowN = odbc_fetch_array($resultN)){
			$row['NOMBRE_VEND'] = $rowN["NOM"];
			}	
    	}
	$cont += 1;
...
if($_POST['area'] == 'Almacen'){		
	$result = odbc_exec($db2conp, $sql) ; echo odbc_errormsg();
		while($row = odbc_fetch_array($result)){
			...
			$tiPRO["$campo"][]= utf8_encode($valor);
			$tiPROT["$campo"] += utf8_encode($valor);
			}
		}	
}
//print_r($tiPRO);		
