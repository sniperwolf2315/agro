...
//$sql="SELECT * FROM AGR620CFAG.SROPRG SRBPRG LEFT JOIN AGR620CFAG.SROEAN SRBEAN ON SRBPRG.PGPRDC = SRBEAN.PJPRDC WHERE SRBEAN.PJEANP LIKE '%4540104902016%'";
                ...
                if($compania=='Agrocampo' || $compania=='AgrocampoN'){
                    ...
                    if($tam==13){
                        ...
                        $sql="SELECT * FROM AGR620CFAG.VISINVWHM WHERE PCXPRC='$codigotemp' OR PGPRDC='$codigotemp'  OR LEFT(PCXPRC,12)='$codigotemp'";
                    }
                    if($tam==12 || $tam==11){
                        ...
                        $sql="SELECT * FROM AGR620CFAG.VISINVWHM WHERE PCXPRC='$codigotemp' OR PGPRDC='$codigotemp'  OR LEFT(PCXPRC,12)='$codigotemp' OR LEFT(PCXPRC,11)='$codigotemp'";
                    }
                }else if($compania=='Comervet'){
                    require_once('conexionInvComervet80.php');
                    if($tam==13){
                        //$sql="SELECT DISTINCT PGPRDC AS ITEM FROM AGR620CFAG.VISINVWHM WHERE PCXPRC='$codigotemp' OR LEFT(PCXPRC,12)='$codigo2' OR LEFT(PCXPRC,12)='$codigotemp'";
                        $sql="SELECT * FROM AGR620CFZZ.VISINVWHM WHERE PGPRDC='$codigotemp' OR LEFT(PGPRDC,12)='$codigotemp'";
                    }
                    if($tam==12 || $tam==11){
                        //$sql="SELECT DISTINCT PGPRDC AS ITEM FROM AGR620CFAG.VISINVWHM WHERE PCXPRC='$codigotemp' OR LEFT(PCXPRC,11)='$codigo3' OR LEFT(PCXPRC,12)='$codigotemp'";
                        $sql="SELECT * FROM AGR620CFZZ.VISINVWHM WHERE PGPRDC='$codigotemp' OR LEFT(PGPRDC,12)='$codigotemp' OR LEFT(PGPRDC,11)='$codigotemp'";
                    }
                }
