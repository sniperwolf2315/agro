...
//(IDordenAgro='' OR Estado='1') AND
    ...
    //echo "SELECT Sequence, IDPedidoPagina, IDCliente, IDordenAgro, Celular, FechaIngreso FROM [sqlFacturas].[dbo].[CreacionEncabezadoVenta] WHERE (IDordenAgro!='' OR Estado='0') AND Sequence='$Sequence'";
    //exit();
    if($resultado = mssql_fetch_array($resultSQL)){
        ...
        $sql = "SELECT * FROM AGR620CFAG.SRBSOH WHERE OHORDT IN('S1','S5') AND (OHOREF='$Sequence' OR OHORNO='$Sequence')  AND (OHODAT BETWEEN '$FechaSQL' AND '$FechaSys')";
        $result = odbc_exec($db2con, $sql);
        $rc=odbc_num_rows($result);
        //echo "SELECT * FROM AGR620CFAG.SRBSOH WHERE OHORDT IN('S1','S5') AND (OHOREF='$Sequence') AND (OHODAT BETWEEN '$FechaSQL' AND '$FechaSys'";
        //exit();
    ...
    mssql_close();
                ...
                //write_visita ($texto);
    if($accion==""){
        $accion="Orden No. ".$Sequence." no encontrada en IBS_";
    }
    echo $accion;
?>