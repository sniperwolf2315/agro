...
//AND datepart(Month,FechaIngreso)='03'
                ...
                */
                ...
                $objWorkSheet = $objPHPExcel->createSheet(0);
                    ...
                    $objWorkSheet->setCellValue('A2', '#')
                        ...
                        ->setCellValue('I2', 'Estado Ped')
                        ->setCellValue('J2', 'Analisis');
    ...
    while($q<$conSeq){
     $resultSQL = mssql_query("SELECT Sequence, Estado, IDPedidoPagina, IDCliente, NombreCliente, IDordenAgro, Celular, CodigoMunicipalidad, FechaIngreso FROM [sqlFacturas].[dbo].[CreacionEncabezadoVenta] WHERE Sequence='$arrarSequence[$q]'");
            ...
            while($row = odbc_fetch_array($result)){
                ...
                if($Orden != ""){
                    //SI ORDEN ESTA EN EL ENCABEZADO
                                ...
                                if($NombreClienteIBS==""){
                                    $r=$r . "<td style='$S2'>".$NombreClienteSQL."</td>";
                                }else{
                                    $r=$r . "<td style='$S2'>".$NombreClienteIBS."</td>";
                                }
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('G'.$fd, $NombreClienteSQL)
                    ->setCellValue('H'.$fd, $IDestino)
                    ->setCellValue('I'.$fd, $Estado)
                    ->setCellValue('J'.$fd, "Falta en IBS");
