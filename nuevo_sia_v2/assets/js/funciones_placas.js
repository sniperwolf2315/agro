...
function cronometro(var1, var2, var3) {
  ...
  function exportar_excel(nombre_tabla, nombre_archivo = '') {
    ...
    /** se establece la hora actual del ssistema para asignar y ver la generacion del informe */
    let hoy = new Date();
  ...
  function actualiza_datos(id,cedula,nombre,placa,empresa,activo) {
    
 
    if(cedula.trim()=="0000000000" || nombre=="SIN_DATO" || empresa.trim()=="SIN_DATO"){
        alert("No se aceptan datos vacios");
    }
    ...
    peticion_http.open('POST', `actualiza_placa.php?id_doc=${id}&doc_act=${cedula}&nom_act=${nombre}&plc_emp=${placa}&emp_car=${empresa}&activo=${activo}`, true);
...
function inserta_datos(id,cedula,nombre,placa,empresa) {
    ...
    console.log(id,  cedula,nombre,placa,empresa);
    ...
    peticion_http.onreadystatechange = muestraContenido;
    // Realizar peticion HTTP
    peticion_http.open('POST', `crear_placa.php?id_doc=${id}&doc_act=${cedula}&nom_act=${nombre}&plc_emp=${placa}&emp_car=${empresa}`, true);
