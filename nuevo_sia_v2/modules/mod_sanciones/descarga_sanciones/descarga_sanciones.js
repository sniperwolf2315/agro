...
if (btn_cons_gh !== null) {
    btn_cons_gh.addEventListener('click', function () {
        ...
        const valores = window.location.search;
        campo = obtner_variables_url('area');
        renderizar('descargar_sancion','',`?area=${campo}`);
    });
...
function consultar_query(){
    ...
    if(fecha_desde=='' &&fecha_hasta=='' ){
        ...
        */
       retorna_consulta= `select * from API_SANCIONES_AGRO order by SAN_ACTIVA,FECHA_REG`;
    }else{
        /*
        if(area_consulta_cod=='SEGURIDAD' || area_consulta_cod=='SISTEMAS' || area_consulta_cod=='AUDITORIA'){
        ...
        */
       retorna_consulta= `select * from API_SANCIONES_AGRO where CONVERT(nvarchar(30),FECHA_REG, 111) between '${fecha_desde}' and '${fecha_hasta}' order by  SAN_ACTIVA,FECHA_REG desc`;
    }
    
    renderizar('info_tabla_sancion','0',`?v1=${retorna_consulta}&area=${area_consulta}&area_cod=${area_consulta_cod}`);
}
