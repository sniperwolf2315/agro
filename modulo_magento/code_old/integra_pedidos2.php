...
if (mysqli_connect_errno())
  { echo "Failed to connect to MySQL MAgento: " . mysqli_connect_error()
          ."<BR>host 	= 	'$localhostL'	<br> 	usuario 		= 	'$userA'	<br>
            clave		=	'$claveO' <br> 	base_datos	=	'$base_datosL'
            <br>
            <BR>
            new mysqli($localhostL,$userA,$claveO,$base_datosL);
            "
          ; }   
  else{
  echo "<br>Conecto<br>"; //die;
  }
  die;
*/
//mmsql AgroC
...
$sql ="SELECT 
    ...
    , if(status = 'ondelivery','contra','') AS Pago
...
while($row = mysqli_fetch_assoc($result)){ print_r($row);
 ...
 $sqlINSERT[] = "INSERT INTO 2magento_orden ($campos) VALUES ('$valores'); ";
 $campos =''; $valores='';
}
...
while($row = mysqli_fetch_assoc($result)){
 ...
 $sqlINSERTitem[] = "INSERT INTO 2magento_orden_item ($campos) VALUES ('$valores'); ";
 $campos =''; $valores='';
}
...
mysqli_query($mysqliL, "DELETE FROM 2magento_orden");
...
mysqli_query($mysqliL, "DELETE FROM 2magento_orden_item");
...
//buscar
$sql = "SELECT * FROM 2magento_orden_item WHERE ValorItems > 0 OR IDProducto IN( '818142136','818190475','818142033','868620691','868620692','838310058')"; //,'868620691','868620692' 
$result = $result = mysqli_query($mysqliL, $sql);
while($row = mysqli_fetch_assoc($result)){
  $idPP = $row[IDPedidoPagina];
  $comaID =',';
...
if(strlen($encabezados) > 0 ){
 mysqli_query($mysqliL," INSERT INTO 2magento_orden_log$prueba (reg_guardados, hora_i, cant_reg) VALUES ('$encabezados', '$hini' , '$contEncabezados')") ;
}
