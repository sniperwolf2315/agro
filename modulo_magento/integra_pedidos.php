<!-- http://192.168.6.55/modulo_magento/integra_pedidos.php -->
...
// include('../_lupap.php');/* OJO ESTE YA CONTIENE LAS CREDENCIALES PARA CONSULTAR EL LUPAP RENOVACION */
$logs = new log();
...
if($prueba =='TMP'){
  ...
  echo "<center> <font size='5' color='green'>PRODUCCION HABILITADO</font></center><br><br>"; 
}
...
/** SERVIDOR DE DESARROLLO MAGENTO  IBS  RAZON SOCIAL TABLA   bduufggxgg*/
/*
$localhostL	  = '164.92.100.82'; 	
...
/* ESTE SELECT YA INCORPORA EN LA BASE SBMCUUT NO EXISTE LA TABLA AADDO EN BDUUFGG
1 RAZON SOCIAL
2 GRUPO COMPRADORES
3 ENVIO EXPRES
4 FRANJAS HORARIAS
...
// aaddo.date as FechaEntrega , /* aun no creado en prod */
// aaddo.time_from as Ruta,     /* aun no creado en prod */
// aaddo.comment as Comentario, /* aun no creado en prod */
// (case WHEN acei.value  is not null then acei.value  else aacag.id_type end ) TipoDocumento
// FROM
// agro_sales_order A
...
// LEFT JOIN agro_amasty_deliverydate_deliverydate_order aaddo ON aaddo.order_id = A. entity_id /* AUN NO CREADO EN PROD */
...
// and (if(customer_taxvat IS NULL,vat_id,customer_taxvat) REGEXP '^[0-9]+$' = 1)
...
A.shipping_description as tipoenvio ,
ifnull(A.customer_group_id, 0) as group_id ,
'0000-00-00 00:00:00.000' as FechaEntrega ,
'0' as Ruta,
'' as Comentario,
(CASE WHEN acei.value  is not null then acei.value else aacag.id_type end) TipoDocumento
FROM
...
LEFT JOIN agro_customer_entity_varchar acev 	  			      ON acev.entity_id = A.entity_id 
LEFT join agro_amasty_customer_attributes_guest aacag 		  on aacag.order_id = A.entity_id
WHERE (( A.status = 'processing'	AND substr(C.comment, 1, 8)= 'APPROVED' )	OR A.status = 'ondelivery' ) 
and (if(customer_taxvat IS NULL,vat_id,customer_taxvat) REGEXP '^[0-9]+$' = 1)
and A.created_at >='$hoy_1sem'
$no_incluir_clausula
"; 
...
while($row = mysqli_fetch_assoc($result)){
  ...
  foreach($row AS $titulo => $valor){
    ...
    /* SOLO EVENTUAL  SOLICITA JUNAPALBO EL 07072023
      PARA PRODUCTO DE NEXGARD QUE DESDE LA PAGINA DEBEN VISUALIZAR UN REFERENCIA PERO AL PASAR A IBS DEBE LEER LA ORIGINAL CON EL DCTO YA COORDINADO CON COSTOS
      */
      switch ($valor){
        case '5151151105110440001':
          $valor = '5151151105110440096';
          break;
        case '5151151105110440002':
              $valor = '5151151105110440097';
            break;
        case '5151151105110440003':
              $valor = '5151151105110440098';
            break;
        case '5151151105110440004':
              $valor = '5151151105110440099';
            break;
        default:
              $valor = $valor;
            break;
...
foreach($sqlINSERT AS $idPP => $ins){
  ...
  echo $error.'<br>'; 
  mysqli_query($mysqliL,"INSERT INTO magento_orden_error (id, error) VALUES ('$idPP', '$error'); <br>");
  }
}
...
while($rowMS = mssql_fetch_row($resultMS)){
  echo "UPDATE magento_orden$prueba_mysql SET vBarrio = 'ya' WHERE IDPedidoPagina ='$rowMS[0]';";
...
while($row = mysqli_fetch_row($result)){
    ...
    $ciudad= remove_characters( $ciudadaBuscar );
      ...
      if (!mssql_num_rows($SqlLupa)) {
            //CODIGO POSTAL LUPAP la funcion CALLAPI se conecta con el servicio de lupap y consulta el codigo postal
            $resultLU = geocode($ciudad, $direccion);
            // $resultLU = CallAPI($ciudad, $direccion);
...
$direabuscar="";  //nuevo
$codPostLupap=""; //nuevo
/**EN ESTE SELECT VAMOS A LLAMAR LOS CAMPOS DE magento_orden A INSERTAR A SQL SERVER */
$sql = "SELECT IDPedidoPagina,IDCliente,NombreCliente,Estado,Direccion,Telefono,Celular,CodigoMunicipalidad,Email,IDordenAgro,IDestadoAgro,IDDescEstado,IDFacturaAgro,ValorOrden,ValorFlete,vBarrio,Sequence,Fecha,Pago,Descuento,Notas, group_id,FechaEntrega,Ruta,Comentario,TipoDocumento,CodigoPosLupap FROM magento_orden$prueba_mysql ";
$result = $result = mysqli_query($mysqliL, $sql);
while($row = mysqli_fetch_assoc($result)){
  $idPP = $row[IDPedidoPagina];
  $comaID =',';
  ...
  foreach($row AS $titulo => $valor){
    ...
    /* SI LA VARIABLE (TITULO = COLUMNA)  ES = CodigoPosLupap SU VALOR ES LA VARIABLE CODIGO POSTAL DEVUELTA POR LUPAP / SIEMPRE DEBE PASAR PO EL API*/
...
//PRIMER INSERT A LA TABLA DE ENCABEZADO SIA
 ...
 /* ACTUALZIAR COLUMNA CODIGO POSTAL LUPAP DE LA TABLA SIA  CreacionEncabezadoVenta*/
$sql_codigo_postal_lupap = "
";
...
while($row = mysqli_fetch_assoc($result)){
 ...
 $campos =''; $valores='';
  ...
  //  crear_logs($nombre_arch, $msqlINSERTitem);
}
...
/*SCRIPT OPARA ACTUALZIA EL CAMPO CODIGO POSTAL LUPAP AL FINALIZAR EL REGISTRO */
include_once('_actualiza_codpostal.php');
/*SCRIPT OPARA ACTUALZIA EL CAMPO CODIGO POSTAL LUPAP AL FINALIZAR EL REGISTRO */
...
WITH C AS
 ...
 ,IDOrdenAgro
 ,Fecha
 ,ROW_NUMBER() OVER (PARTITION BY IDPedidoPagina ORDER BY IDPedidoPagina) AS DUPLICADO
 --FROM CreacionItemsVenta$prueba 
 FROM CreacionEncabezadoVenta$prueba 
  WHERE YEAR(Fecha)=$año_actual  AND MONTH(Fecha)=$mes_actual 
 )
