...
if($_POST['completoPROD'] == 'SI'){
  ...
  mysqli_query($mysqli,"DELETE FROM prov_plan_det_art WHERE id_plan_det = '$_POST[maxId]' ");
  // guarda datos del formulario  
  for($i = 0; $i <= $_POST['prods']; $i++){
    if($_POST["art$i"] != ''){
      $grabe ++; 
      $prods = explode("|", $_POST["prod$i"]);
      $sql = "INSERT INTO prov_plan_det_art (id_plan_det, CODIGO, NOMBRE, COSTO, MADRE) VALUES ( '$_POST[maxId]', '$prods[0]', '$prods[1]', '$prods[2]', '$prods[3]');";
...
$sql ="SELECT PROVEEDOR, AÃ‘O, GRUPO, PAGO, DETALLE, PRESUPUESTO
       FROM prov_plan_det
       LEFT JOIN prov_plan ON prov_plan.id = id_plan
       WHERE 
       prov_plan_det.id = '$_POST[maxId]'
      ";
...
$sql ="SELECT NOMBRE, MADRE FROM prov_plan_det_art WHERE id_plan_det = '$_POST[maxId]' ";
...
$sql ="SELECT
       ...
       AND SRBPRG.PGPGRP IN ('$titDet[GRUPO]')
       $fprod
...
<table border="1" class="frr" width="99%">
  <tr>
    <td align="center" colspan="<?= $cols?>">
      ...
      <br>
      <? echo " Prov: <b>$titDet[PROVEEDOR]</b>. Año: <b>$titDet[AÑO]</b>. Grupo: <b>$titDet[GRUPO]</b>. Pago: <b>$titDet[PAGO]</b> - <b>$titDet[DETALLE]</b>. Presup: <b>".number_format($titDet[PRESUPUESTO],0,',','.')."</b>"; ?>  
      <br>
      <?
      if(count($itemGuardado) > 0){
