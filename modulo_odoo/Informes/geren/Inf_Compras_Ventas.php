...
while($m < $numan){
    $QueryG="select
        ...
        group by c2.name, split_part(c1.name, '-', 1),u.login
        having c2.name is not null
        AND c2.name not like '%CONTABILIDAD%' AND c2.name not like '%SERVICIOS%' AND c2.name not like '%ACTIVOS FIJOS%'
        and u.login in('$logginB[$m]')
        order by split_part(c1.name, '-', 1) asc
    ";
    ...
    foreach($datosC as $datoC){
        ...
        $QueryNom="select trim(case when substr(c1.name,4,2)='- ' then substr(c1.name,8,40) else (case when substr(c1.name,5,2)!=' ' then substr(c1.name,5,40) else substr(c1.name,7,40) end)  end) as desc_grupo
            from product_category c1
            where split_part(c1.name, '-', 1) ='$grp' limit 1
        ";
        $dgrp="-";
...
$QueryV="select
    ...
    HAVING
    o.state='done'
    AND EXTRACT(YEAR FROM  cf.date_invoice)  = '$anioReport' AND EXTRACT(MONTH FROM  cf.date_invoice) = '$mesReport'
    ...
    $QueryC="select distinct
            ...
            HAVING EXTRACT(YEAR FROM  o.date_approve)  = '$anioReport' AND EXTRACT(MONTH FROM  o.date_approve) = '$mesReport'
    ...
    while($m2 < $numGrp){
        ...
        foreach($datosP as $datoP){
            ...
            //echo $productoInv[$i]."--".$grpProduInv[$i]."--".$manProduInv[$i]."</br>";
            $i++;
        }
        $m2++;
    }
    ...
    while($i<$cantProdInv){
        ...
        from stock_quant q
                ...
                left join res_users ru ON pt.product_manager=ru.id
        WHERE q.product_id='".$idProd."' and ru.login='".$manInv."' and q.location_id NOT IN('8','9') limit 1
        ";
        ...
        foreach($datosI as $datoI){
            ...
            echo $aniInvF."^^".$aniInvI."^^<br>";
            //no tiene en cuenta el mes actual
            //si año mov es > reporte solicitado
            ...
            echo $aniInvI."....".$aniInvF."....".$mesInvI."....".$mesInvIF.";  ";
        }
        ...
        if($manInv!=''){
            ...
            $queryInvI="select
                ...
                and EXTRACT(YEAR FROM  in_date)  = '$aniInvI' AND EXTRACT(MONTH FROM  in_date) <= '$mesInvI'
            ...
            //echo "</br>".$grpInvI." -- Incial: ".$idProd."--".$cantid;
            //echo $mesInvIF."</br>";
            //INV FINAL
            ...
            $queryInvF="select
                ...
                group by ru.login,product_id,in_date,location_id
                having product_id='".$idProd."' and ru.login='$manInv'
                and EXTRACT(YEAR FROM  in_date)  = '$aniInvF' AND EXTRACT(MONTH FROM  in_date) = '$mesInvIF'
                and location_id NOT IN('8','9')
            ";
