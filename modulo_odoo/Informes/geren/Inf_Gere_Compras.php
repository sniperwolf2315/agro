...
$query1="select o.name as orden, p.default_code as item, p.name_template as nombre,
case when cf.number is null then cf.supplier_invoice else cf.number end as factura,il.local_subtotal as valor,
il.quantity as cantidad,left(cf.origin,3) as bodega,cf.date_invoice as fecha,o.state as estado
from product_product p
left join purchase_order o ON p.document_purchase=o.name
left join purchase_invoice_rel oi ON o.id=oi.purchase_id
left join account_invoice cf ON oi.invoice_id=cf.id --and o.state='approved'
left join account_invoice_line il ON cf.id=il.invoice_id and p.id=il.product_id
where cf.date_invoice between '".$feini."' and '".$fefin."';";

$resultado1= $Conn->prepare($query1);
$resultado1->execute();
$datos1=$resultado1->fetchAll();
$r=$r."<p style=\"text-align: center;\" class=\"z-depth-1\">Informe General de Compras</p>";
