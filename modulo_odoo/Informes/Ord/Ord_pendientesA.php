...
$query1 = "select scrld.qty,scrld.state,
                    ...
                    where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and scr.state='progress';";
$query2 = "select scrld.qty,scrld.state,
                    ...
                    where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and pcl.code='GANADERIA';";
$query3 = "select scrld.qty,scrld.state,
                    ...
                    left join stock_picking as sp on scrld.picking_id=sp.id
                    left join purchase_order_type as pot on scrld.purchase_type_id=pot.id
                    where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and scr.state='cancel';";
$query4 = "select scrld.qty,scrld.state,
                    ...
                    left join stock_picking as sp on scrld.picking_id=sp.id
                    left join purchase_order_type as pot on scrld.purchase_type_id=pot.id
                    where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and scr.state='done';";
...
//echo "dias: {$dia} <br>"."Fecha inicial: ".$feini."<br>"."fecha Fin: ".$fefin;
        ...
        <a href='Informexls/ORDENES_PENDIENTES_005.xlsx' class=\"z-depth-1 white-text text-darken-2\">Descargar</a><Strong></td>";
        $r=$r."</tr>";

        $fd=3;
        // $r="Informexls/Informe_Inventario008.xlsx"
        $miruta='../Informexls/';
        $nombre_fichero = 'ORDENES_PENDIENTES_005';
            ...
            if(file_exists($mipath2)) {
                ...
                for($hojH=0; $hojH<6; $hojH++){
                    ...
                    if($conH==0){
                        ...
                        $objWorkSheet->setTitle('Progreso_005');
                    }else if($conH==1){
                        ...
                        $objWorkSheet->setCellValue('A2', 'No.')
                                   ...
                                   //colocar titulos a las hojas de excel
                                   //$objWorkSheet->setTitle("$i");
                        $objWorkSheet->setTitle('Ganaderia_005');
                    }else if($conH==2){
                        ...
                        $objWorkSheet->setCellValue('A2', 'No.')
                                   ...
                                   //colocar titulos a las hojas de excel
                                   //$objWorkSheet->setTitle("$i");
                        $objWorkSheet->setTitle('Cancelado_005');
                    }else if($conH==3){
                        ...
                        $objWorkSheet->setCellValue('A2', 'No.')
                                   ...
                                   //colocar titulos a las hojas de excel
                                   //$objWorkSheet->setTitle("$i");
                        $objWorkSheet->setTitle('Transferido_005');
                    }else if($conH==4){
                        ...
                        $objWorkSheet->setCellValue('A2', 'No.')
                                   ...
                                   //colocar titulos a las hojas de excel
                                   //$objWorkSheet->setTitle("$i");
                        $objWorkSheet->setTitle('Pets_005');
                    }else if($conH==5){
                        ...
                        $objWorkSheet->setCellValue('A2', 'No.')
                                   ...
                                   //colocar titulos a las hojas de excel
                                   //$objWorkSheet->setTitle("$i");
                        $objWorkSheet->setTitle('Laboratorios_005');
        ...
        for($queB=0; $queB<6; $queB++){
            ...
            if($queA==4){
                ...
                if($resultadocat = mssql_fetch_array($resultSQL)){
                    ...
                    left join purchase_order_type as pot on scrld.purchase_type_id=pot.id
                    left join product_category_level pcl ON pt.category_1_id=pcl.id
                    where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and pp.default_code like '%$grupo%';";//sw.code='008' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and
                ...
                $j=0;
                    foreach($datos6 as $dato6){
                        ...
                        //$r=$r."<p>".$query2."Ordenes de Compra Pendientes Por Entregar a Corte1: ".$pro."</p>";
                            ...
                            left join stock_picking as sp on scrld.picking_id=sp.id
                            left join purchase_order_type as pot on scrld.purchase_type_id=pot.id
                            where sw.code='005' and sp.name like '%IN%' and scrld.state in ('transferred','returned') and rp.ref='$pro';";
