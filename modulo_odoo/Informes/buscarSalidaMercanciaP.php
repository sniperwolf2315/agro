...
if ($tipo_consulta === 'normal') {
    $filtro = "and extract(year from so.date_order)  = $anio and extract(month from so.date_order) = $mes ";
} else if ($tipo_consulta === 'rangos') {
    $filtro = "and extract(year from so.date_order)  between $anio and $anio_hasta and extract(month from so.date_order) between $mes and $mes_hasta and extract(day from SO.DATE_ORDER ) BETWEEN $dia and $dia_hasta ";
} else {
...
select 
    so.id as id_orden,
    so.date_order as fec_orden,
    so.state as state_orden,
    so.name as orden,
    sw.name as ubi_bodega
from 
    sale_order as so
...
$cantP = count($datos);
if ($cantP === 0) {
    echo '¡Esta consulta no tiene datos quue visualizar!';
    return;
}
...
foreach ($datos as $dato) {
    ...
    $query = "
                select 
                    ai.sale_id as id_orden
                    ,ai.number as factura
                    ,qt.name as tipo
                    ,pp.default_code as item
                    ,pt.name as descrip
                    ,spo.product_qty as cantidad
                    ,sdl.create_date as hora_final
                    ,rp.street as destino
                    ,sdl.vehicle_char as transportador
                FROM 
