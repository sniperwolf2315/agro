...
$query1="select
      ...
      (case when pk.state='done' then 'transferido' else (case when pk.state='waiting' then 'esperando otra operacion' else (case when pk.state='confirmed' then 'esperando disponibilidad' else (case when pk.state='assigned' then 'listo para transferir' else (case when pk.state='partially_available' then 'parcialmente disponible' else (case when pk.state='draft' then 'borrador' else pk.state end) end) end) end) end) end) as estado,
      --separate_date as fecha_separacion,
      left(cast(separate_date as varchar),16) as fecha_separacion,
      (select res_partner.name from res_users inner join res_partner on res_partner.id=res_users.partner_id where res_users.id=separated_by) as usuario_separo,
       (case when pk.state='done' then (select count (stock_pack_operation.id) from stock_pack_operation inner join product_product on product_product.id=stock_pack_operation.product_id inner join product_template on product_template.id=product_product.product_tmpl_id where stock_pack_operation.picking_id=pk.id and product_template.type not like 'service' ) else (select count (stock_quant.id) from stock_move inner join stock_quant on stock_quant.reservation_id=stock_move.id where stock_move.picking_id=pk.id ) end ) as cantidad,
       (select purchase_order.name from purchase_order where purchase_order.name=pk.origin limit 1 ) as documento_compra,
      (select sale_order.name from sale_order where sale_order.name=pk.origin limit 1 ) as documento_venta
    from stock_picking pk
    inner join stock_picking_type t on t.id=pk.picking_type_id
    where pk.state not like 'cancel'
