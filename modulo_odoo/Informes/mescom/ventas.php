...
$areax= array("VENTA EXTERNA","CONCENTRADOS","GATOS","MOSTRADOR","IMPORTADOS","SEMILLAS  Y FERRETERIA","VACUNACION","CANALES DIGITALES","PEQUE�OS");
// $areax= array("VENTA EXTERNA","CONCENTRADOS","GATOS","MOSTRADOR","IMPORTADOS","IMPORTADOSPM","SEMILLAS  Y FERRETERIA","VACUNACION","CANALES DIGITALES","PEQUE�OS");
    ...
    while($row1 = mssql_fetch_array($queryv)){
        ...
        if($area=='MOSTRADOR'){
            $queryventas = mssql_query("
              ...
              FROM 
                [sqlFacturas].[dbo].[FACDETALLEFACTURANEW] f
             ...
             WHERE 
                p.Codigo='$periodo' AND f.Vendedor='$vend' and f.call='VEND999' GROUP BY f.Vendedor;");
        }else{
            $queryventas = mssql_query("
              SELECT 
                Vendedor, 
              ...
              FROM 
                ...
                AND FechaOrden <= CONVERT(VARCHAR, p.FechaFin) 
             WHERE 
                p.Codigo='$periodo' AND f.Vendedor='$vend' GROUP BY f.Vendedor;");
        }
        
        
        
         //$queryventas = mssql_query("SELECT ValorSinIVA FROM [sqlFacturas].[dbo].[facDetalleFacturanew] WHERE Vendedor='$vend' AND Call='VANANDELL' AND Manejador='VANANDELL';", $cLink);
