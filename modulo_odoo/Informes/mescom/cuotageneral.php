...
if($tipo_consulta =='normal' ){
    $anexo_texto =" AND IDPER='$periodo'" ;
}else if($tipo_consulta =='rangos'){
    $anexo_texto =" AND IDPER between '$fec_desde' and '$fec_hasta'" ;
}
else{
    $anexo_texto =" AND IDPER= '$periodo'" ;
}
...
while($row1 = mssql_fetch_array($queryv)){
    ...
    $cuotagen=0;
                                        // #LLAMA LA CUOTA GENERAL DESDE IBS
    $sqlIBS = "SELECT IDTCUOTA, IDVEND, IDPER, VCUOTA FROM AGR620CFAG.VENDCUOTA WHERE IDTCUOTA IN('CTGG','CTGO','CTDM') AND IDVEND = '$vend'  $anexo_texto ";   
    ...
    if($num > 0){
        while($row = odbc_fetch_array($result)){
            ...
            if (!mssql_num_rows($query2)) {
                /** SOLO SE INSERTAN VALORES DE LA CUOTA GENERAL EL RSTANTE DE CAMPOS VALOR = 0  */
