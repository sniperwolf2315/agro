...
$resultSQL = mssql_query("SELECT* FROM [InformesCompVentas].[dbo].[InfItemsMinAgricultura] ORDER BY Manejador DESC",$cLink);
...
while($resultadoSql = mssql_fetch_array($resultSQL)){
    ...
    $resultSQL2 = mssql_query("SELECT ItemOdoo FROM [InformesCompVentas].[dbo].[InfItemsIbsOdoo] WHERE ItemIbs='$itemIbs'",$cLink);
    //echo $itemIbs."----";
    if($resultadoSql2 = mssql_fetch_array($resultSQL2)){
        ...
        $queryExist="select
            ...
            group by p.default_code,q.location_id,l.complete_name
            having p.default_code='$itemOdoo' and q.location_id not in('9')
        ";
