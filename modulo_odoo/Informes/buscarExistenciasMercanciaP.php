...
/*
        TODO:
        hacer la relacion con la tabla  left outer join product_category_level n ON n.id=p.product_tmpl_id  y sacar la categoria o consultar si se encuentra en otra tabla
*/
...
$cabecera = array('No', 'Id', 'CodProdOdoo', 'CodProdIBS', 'NomProd', 'CostUnd', 'CostTot', 'Cant', 'Bod', 'Locacion', 'Man', 'Grupo', 'NomGrup', 'NomCateg', 'Descargar');
$r = "
              <h5> 
               <strong>
                        <p style=\"text-align: center;\" class=\"z-depth-1\">
                            Existencias Bodega Portos.
                        </p>                                     
                </strong> 
              </h5>
          ...
          ";
foreach ($cabecera as $titles) {
    if ($titles === 'Descargar') {
        $r = $r . " 
        <td style=\"font-weight: bold;text-align: left;\" class=\"z-depth-1 white-text text-darken-2\">
            <Strong>
                <a href='Informexls/Informe_Inventario008.xlsx' class=\"z-depth-1 white-text text-darken-2\">
                Descargar
                </a>
            <Strong>
        </td>";
    } else {
        $r = $r . "  <td style=\"font-weight: bold;text-align: left;\" class=\"z-depth-1 white-text text-darken-2\">$titles</td>";
    }
}
$r = $r . "</tr>";
...
if (file_exists($miruta)) {
    ...
    if (file_exists($mipath2)) {
        ...
        // $objWorkSheet->getActiveSheet()->mergeCells('A1:O1');
...
foreach ($datos as $dato) {
    ...
    // $objWorkSheet = $objPHPExcel->removeSheetByIndex(
