...
";
if ($consulta == 'normal') {
    $sql_where = "  EXTRACT(MONTH FROM  o.date_approve) HAVING EXTRACT(YEAR FROM  o.date_approve)  ='" . $anioReport . "' 
                    AND EXTRACT(MONTH FROM  o.date_approve) = '" . $mesReport . "'
                    and cf.supplier_invoice is not null and trim(u.login) is not null
                    and ol.state='done'
            ";
} else {
    $sql_where = "  EXTRACT(MONTH FROM  o.date_approve) HAVING EXTRACT(YEAR FROM  o.date_approve) between $anio_desde and $anio_hasta 
