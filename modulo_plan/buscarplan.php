<?php
            ...
            if($emp=='AG'){
                $result = mssql_query("SELECT * from zAgroPremios2021 WHERE Cedula='$d1' and Empresa='$emp'");
            }else if($emp=='X1'){
                $result = mssql_query("SELECT * from zAgroPremiosPestar2021 WHERE Cedula='$d1' and Empresa='$emp'");
            }
            ...
            if($resultado){
                 ...
                 $datos=utf8_encode($d0).'^'.utf8_encode($d1).'^'.utf8_decode($d2).'^'.utf8_decode($d3).'^'.utf8_decode($d4).'^'.$d5.'^'.$d6.'^'.$d7.'^'.utf8_encode($d8).'^'.$d9.'^'.$d10.'^'.$d11.'^'.$d12.'^'.$P1.'^'.$P2.'^'.$P3; 
                 echo $datos;
                 //$mysqli->close(); 
                 //$mssql->close();
