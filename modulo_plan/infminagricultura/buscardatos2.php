...
require('fpdf/fpdf.php');
            ...
            class PDF extends FPDF
            {
                function Header(){
                    ...
                    //cabecera de la tabla
                        ...
                        $this->SetXY(100, 20);
                        $this->Cell(10,7, 'INFORME DE COMPRAS Y VENTAS ACUMULADOS', 0, 'C', 'C','0','');
                        //$this->SetXY(95, 30);
            ...
            while($resultadocat = mssql_fetch_array($resultSQLCat)){
                ...
                while($resultadoTot = mssql_fetch_array($resultSQLTot)){
                    ...
                    //ventas acumuladas
                    $resultSQL = mssql_query("SELECT SUM(COSTO_TOTAL) AS CT, SUM(VLR_EXC_IVA)AS VT FROM [InformesCompVentas].[dbo].[infVentasIbs] WHERE PERIODO BETWEEN '$periodoi' AND '$periodo' AND IDPGRP='$d1'");
                    $resultadoSql = mssql_fetch_array($resultSQL);
                    //costo acumulado
                    ...
                    //compras acumuladas
                    $resultSQL = mssql_query("SELECT SUM(VLR_EXC_IVA) AS CA FROM [InformesCompVentas].[dbo].[infComprasIbs] WHERE PERIODO BETWEEN '$periodoi' AND '$periodo' AND IDPGRP='$d1'");
                    $resultadoSql = mssql_fetch_array($resultSQL);
                    //compras acumuladas
                    //$vcomp=$resultadoSql["VLR_EXC_IVA"];
                    ...
                    if($resultado1){
                     ...
                     //sql server
                    $resultSQL2 = mssql_query("SELECT * FROM infConsolidadoInvSig WHERE GRUPO='$d1' AND PERIODO='$final'");
           ...
           $pdf->SetFont('Arial','B',12);
            ...
            $cod=time()+1;
            $nombrearc="pdf/INFORME_COMPRAS_VENTAS_MES_ACUMULADO.pdf";
