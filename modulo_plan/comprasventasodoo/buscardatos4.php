...
require('fpdf/fpdf.php');
            ...
            class PDF extends FPDF
            {
                function Header(){
                    ...
                    //cabecera de la tabla
                        ...
                        $this->Cell(10,7, 'INFORME DE VENTAS AÑO ANTERIOR vs ACTUAL ACUMULADO ODOO', 0, 'C', 'C', '');
    ...
    while($cm < $tm){
            ...
            while($resultadocat = mssql_fetch_array($resultSQLCat)){
                    ...
                    $resultSQLTot = mssql_query("SELECT CTPPGN, CTPPGD, DESCRIPCION FROM [InformesCompVentas].[dbo].[infPeriodosAcumuladosOdoo] WHERE RESPONSABLE='$nom_maneja' AND DESCRIPCION='$descri' ORDER BY DESCRIPCION ASC");
                    //ciclo ITEMS
                    ...
                    while($resultadoTot = mssql_fetch_array($resultSQLTot)){
                        ...
                        $resultSQLA = mssql_query("SELECT SUM(VLR_EXC_IVA) AS T1 FROM infVentasIbsOdoo WHERE PERIODO BETWEEN '$periodoiant' AND '$periodoant' AND IDPGRP='$d1'");
                        ...
                        //$resultSQLB = mssql_query("SELECT VLR_EXC_IVA FROM [InformesCompVentas].[dbo].[infVentasIbs] WHERE PERIODO='$periodo' AND IDPGRP='$d1'");
                        $resultSQLB = mssql_query("SELECT SUM(VLR_EXC_IVA) AS T2 FROM [InformesCompVentas].[dbo].[infVentasIbsOdoo] WHERE PERIODO BETWEEN '$periodoi' AND '$periodo' AND IDPGRP='$d1'");
    ...
    while($cm < $tm){
            ...
            while($resultadocat = mssql_fetch_array($resultSQLCat)){
                ...
                while($resultadoTot = mssql_fetch_array($resultSQLTot)){
                    ...
                    //ventas año presente $periodo
                    //$resultSQLB = mssql_query("SELECT * FROM [InformesCompVentas].[dbo].[infVentasIbs] WHERE PERIODO='$periodo' AND IDPGRP='$d1'");
                    $resultSQLB = mssql_query("SELECT SUM(VLR_EXC_IVA) AS T2 FROM [InformesCompVentas].[dbo].[infVentasIbsOdoo] WHERE PERIODO BETWEEN '$periodoi' AND '$periodo' AND IDPGRP='$d1'");
                    $resultadoSqlB = mssql_fetch_array($resultSQLB);
                    //$d3=$resultadoSql["COSTO_TOTAL"];
                    //$d4=$resultadoSqlB["VLR_EXC_IVA"];
                    ...
                    if($ejeY>210){
                        ...
                        //VENTAS AÑO ACTUAL
                        $pdf->SetXY (123,$ejeY);
                        $pdf->Cell(25,$ancho, number_format($d4),1, 'C' , 'R' );
                    ...
                    //ventas año actual}
                    $pdf->SetXY (123,$ejeY);
                    $pdf->Cell(25,$ancho, number_format($TotVenIvaT),1, 'C' , 'R' );
                    ...
                    //CRECIMIENTO subtotales por categoria
                    //$CRECT=($DIFT/$TotVenIvaAntT)*100;
                    //($DIF/$d4anterior)*100;
                    $CRECT=($DIFT/$TotVenIvaAntT)*100;
                    $SUBTOTCRECIMIENTO=$SUBTOTCRECIMIENTO+($CRECT);
                    $DIFT=0;
                    $CRECT=round($CRECT,2);
                    $pdf->SetXY (191,$ejeY);
            ...
            //LLAVE AGREGADA CATEGORIAS
                ...
                //ventas año actual
                $pdf->SetXY (123,$ejeY);
                $pdf->Cell(25,$ancho, number_format($TotVenIvaTT),1, 'C' , 'R' );
                ...
                //CRECIMIMIENTO*** Totales por seccion o categoria
                $CRECTT=($DIFTT/$TotVenIvaAntTT)*100;
                $CRECTT=round($CRECTT,2);
                $SUBTOTCRECIMIENTO=round($SUBTOTCRECIMIENTO,2);
                $pdf->SetXY (191,$ejeY);
            ...
            $cod=time()+1;
            $nombrearc="pdf/INFORME_VENTAS_MES_ANIOACT_VS_ANIOANT_ACUMULADO_ODOO.pdf";
